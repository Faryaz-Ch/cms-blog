            ...
            <div class="row">
                ...
                <div class="col-md-9">
                    ...
                    if(isset($_POST['submit'])){
                        $date = time();//detect & use php predefine time.
                        $first_name = mysqli_real_escape_string($con,$_POST['first-name']);
                        $last_name = mysqli_real_escape_string($con,$_POST['last-name']);
                        $username = mysqli_real_escape_string($con,strtolower($_POST['username']));
                        $username_trim = preg_replace('/\s+/','',$username);//It will remove spaces in user name entry.
                        $email = mysqli_real_escape_string($con,strtolower($_POST['email']));
                        $password = mysqli_real_escape_string($con,$_POST['password']);
                        $role = $_POST['role'];
                        $image = $_FILES['image']['name'];//For profile picture
                        $image_tmp = $_FILES['image']['tmp_name'];
                        
                        $check_query = "SELECT * FROM users WHERE username = '$username' or email = '$email'";//Check dublicate entries. 
                        ...
                        if(empty($first_name) or empty($last_name) or empty($username) or empty($email) or empty($password) or empty($image)){
                            ...
                            if(mysqli_query($con,$insert_query)){
                                ...
                                move_uploaded_file($image_tmp, "images/$image");//to move or upload file to server folder.
								...
								//Following will clear Form after successfull submission.
                                $first_name = "";
                                $last_name = "";
                                $email = "";
                                $username = "";
                   ...
                   <div class="row">
                       <div class="col-md-8">
                          <!-- if the form contains a file upload, use enctype="multipart/form-data here -->
                           <form action="" method="post" enctype="multipart/form-data">
                               ...
                               <div class="form-group">
                                   ...
                                   <input type="email" id="email" name="email" class="form-control" value="<?php if(isset($email)){echo $email;}?>" placeholder="Email Address">
                               </div>
